...
class StringManipulation():
    ...
    def words_of_length(self, leng):
        return [x for x in self.words if len(x) == leng]
        
    def words_start_with(self, letter):        
        return [x for x in self.words if x.startswith(letter)]
        
    def longest_word(self):
        return max(self.words, key=len)
        
    def palindromes(self):
        return [x for x in self.words if x == x[::-1]]